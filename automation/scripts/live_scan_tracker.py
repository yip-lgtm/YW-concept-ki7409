...
def check_position_exit(pos, df):
    ...
    for i, (ts, bar) in enumerate(df.iterrows()):
        ...
        # LL-iter 2026-08-28: avg R -0.138, all T1+SL only. Move close to T2 (1.618R).
        if hit_sl:
            exit_price = sl
            exit_level = "SL"
            R_multiple = -1.0
    ...
    if closed:
            ...
            try:
                if dedup_file.exists():
                    last_sent = dedup_file.read_text().strip()
            except: pass
            if last_sent != sent_key:
                send_tg("\n".join(lines))
